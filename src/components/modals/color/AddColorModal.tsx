...
const AddColorModal = ({ token, isModalOpen, setIsModalOpen, setOpenSnackbar }: any) => {
    const dispatch = useDispatch();
	const { register, formState: { errors }, setError, setValue, handleSubmit } = useForm<any>();
    const [colorHex, setColorHex] = useState("#000000");

    const handleAddColor: SubmitHandler<any> = async (data) => {
        try {
            await mainApi.post(
                apiEndpoints.CREATE_COLOR,
			...
			<Modal
				...
				<Box display="flex" flexDirection="column" justifyContent="center" alignItems="center" sx={{
					...
					<Box width="100%" height="90%" sx={{ mt: 4 }}>
                        <form onSubmit={handleSubmit(handleAddColor)} className="w-full flex flex-col">
                            {/* Tên màu */}
							<Box width="100%" sx={{ mb: 4 }}>
                                <TextField fullWidth autoComplete="off" {...register("colorName")} required label="Tên màu" placeholder="Nhập tên màu" />
                            </Box>
